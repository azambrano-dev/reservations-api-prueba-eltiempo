...
`reservations-api/src` es un esqueleto Laravel 12 limpio (sólo el `User` por defecto, `routes/api.php` vacío, migraciones base). La infraestructura ya está montada: nginx + php-fpm 8.3 + MySQL 8.4 en Docker, con `innodb-lock-wait-timeout=10` y un `RESERVATION_RACE_DELAY` ya previsto en `config/php/.env.example`.
...
3. `stock >= quantity` → calcular `$remaining = $stock - $quantity` una sola vez y escribir `UPDATE products SET stock = :remaining` (no `stock - :q`): el valor ya está calculado bajo el lock, y así se ve que la columna `stock` y el `remaining_stock` de la reserva salen literalmente del mismo sitio. `status = confirmed`.
